...
- 信号字段、候选池路由、JSONL 状态、去重和人工确认规则优先遵循 docs/ingestion-normalization.md。
...
必须写入的状态文件：
- state/weekly/YYYY-MM-DD-report.md（YYYY-MM-DD = 本周周日的日期；若运行日就是周日则用运行日）
...
7. §3 下周执行卡优先只引用 confirmed/changed 条目；若必须引用 pending，标题后标注「未人工确认」。

请按以下结构输出，并**完整写入** `state/weekly/YYYY-MM-DD-report.md`（不要写到 state/daily/、仓库根目录或其他路径）：
